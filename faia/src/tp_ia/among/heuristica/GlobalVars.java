...
public class GlobalVars {
    ...
    public static HashMap<String, List<String>> movements = new HashMap<String, List<String>>();
    ...
    public static HashMap<String, List<Integer>> airships(int type){
    	...
    	case 3:
			...
			while(randomValues.size() != totalCrewmembers) {
				
				Integer aux = getRandom(0, airship.size()-1);
				randomValues.add(rooms.get(aux));
			}
			...
			while(randomValues.size() != totalSabotageTask) {
				
				Integer aux = getRandom(0, airship.size()-1);
				if(!randomValues.contains(rooms.get(aux))) {
					randomValues.add(rooms.get(aux));
				}
			}
