...
public class goto12 extends SearchAction{
	 ...
	 @Override
	    public SearchBasedAgentState execute(SearchBasedAgentState s){
	        ...
	        List<Integer> roomValues = state.getAirshipRoomValues(GlobalVars.TWELVE);
	        
	        if (possibleMovements != null && energy > 0) {
	            int index = possibleMovements.indexOf(GlobalVars.TWELVE); 
	            
	            if (index >= 0) {
	                state.setPosition(GlobalVars.TWELVE);
	                if (roomValues.get(0) == -1 && roomValues.get(1) == -1)
	                {
	                	state.setRoomValues(GlobalVars.TWELVE, List.of(0,0));
	                }
	    ...
	    public EnvironmentState execute(AgentState ast, EnvironmentState est) {
	    	...
	    	List<Integer> roomValues = amongState.getAirshipRoomValues(GlobalVars.TWELVE);
	    	...
	    	if(possibleMovements != null && amongEnergy > 0)
	    	{
	    		...
	    		if (index >= 0) {
	    			amongState.setPosition(GlobalVars.TWELVE);
	    			if (roomValues.get(0) == -1 && roomValues.get(1) == -1)
	                {
	                	amongState.setRoomValues(GlobalVars.TWELVE, List.of(0,0));
	                }
	    			amongState.setEnergy(amongEnergy-1);
	    			airshipState.setAgentPosition(GlobalVars.TWELVE);
