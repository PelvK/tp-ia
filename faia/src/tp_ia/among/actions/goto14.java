...
public class goto14 extends SearchAction{
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s){
        ...
        if (possibleMovments != null && energy > 0) {
            int index = possibleMovments.indexOf(AmongAgentState.FOURTEEN); // si ONE esta en la lista, establece la posicion del agente en ONE
            if (index >= 0) {
                agentState.setPosition(AmongAgentState.FOURTEEN);
    ...
    public EnvironmentState execute(AgentState ast, EnvironmentState est) {
    	...
    	{
    		int index = possibleMovements.indexOf(AmongAgentState.FOURTEEN);
    		if (index >= 0) {
    			amongState.setPosition(AmongAgentState.FOURTEEN);
    			amongState.setEnergy(amongEnergy-1);
    			airshipState.setAgentPosition(AmongAgentState.FOURTEEN);
