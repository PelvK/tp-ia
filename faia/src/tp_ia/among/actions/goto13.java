...
public class goto13 extends SearchAction{
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s){
        ...
        if (possibleMovments != null && energy > 0) {
            int index = possibleMovments.indexOf(AmongAgentState.THIRTEEN); // si ONE esta en la lista, establece la posicion del agente en ONE
            if (index >= 0) {
                agentState.setPosition(AmongAgentState.THIRTEEN);
    ...
    public EnvironmentState execute(AgentState ast, EnvironmentState est) {
    	...
    	{
    		int index = possibleMovements.indexOf(AmongAgentState.THIRTEEN);
    		if (index >= 0) {
    			amongState.setPosition(AmongAgentState.THIRTEEN);
    			amongState.setEnergy(amongEnergy-1);
    			airshipState.setAgentPosition(AmongAgentState.THIRTEEN);
