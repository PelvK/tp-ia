...
public class goto6 extends SearchAction{
    ...
    public SearchBasedAgentState execute(SearchBasedAgentState s){
        ...
        if (possibleMovments != null && energy > 0) {
            int index = possibleMovments.indexOf(AmongAgentState.SIX); // si ONE esta en la lista, establece la posicion del agente en ONE
            if (index >= 0) {
                agentState.setPosition(AmongAgentState.SIX);
    ...
    public EnvironmentState execute(AgentState ast, EnvironmentState est) {
    	...
    	{
    		int index = possibleMovements.indexOf(AmongAgentState.SIX);
    		if (index >= 0) {
    			amongState.setPosition(AmongAgentState.SIX);
    			amongState.setEnergy(amongEnergy-1);
    			airshipState.setAgentPosition(AmongAgentState.SIX);
